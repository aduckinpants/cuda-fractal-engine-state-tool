...
only from the neighboring copied bytes.

Packet V6 manifest schema 2 records whether viewport facts were preserved from
the capture and byte-verified against the bound runtime, or generated by that
runtime from the exact copied `state.json`. Older Packet V6 manifests remain on
disk but must be rebuilt before new proof work.

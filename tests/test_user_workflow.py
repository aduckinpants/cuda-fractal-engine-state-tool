...
class UserWorkflowTests(unittest.TestCase):
    ...
    def test_real_import_summary_and_exact_packet_persistence(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("signal-processing authority", packet.packet_text)
            self.assertIn("not recurrence authority", packet.packet_text)
            self.assertIn("A controlled comparison may prove visible contribution", packet.packet_text)
            self.assertIn("recurrence, orbit geometry, convergence", packet.packet_text)

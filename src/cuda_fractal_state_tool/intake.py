...
def build_intake_packet(baseline_manifest_path: Path, replay_state_path: Path) -> str:
    ...
    return "\n".join(
        [
            ...
            "Treat runtime describe outputs as authority for lane/function metadata.",
            "If runtime metadata cache is unavailable or stale, refresh from runtime before lane contract decisions.",
            "Keep source schema as supplementary provenance only; do not override runtime-observed metadata with source-only assumptions.",
            "",
            ...
            "",
            "9. Phase 3 draft/lane authoring constraint",
            "Do not author color_pipeline_draft in proposal_v1 yet.",
            "Only propose bounded scalar and coupled color-triplet overrides until the Phase 3 lane contract and validation helpers are merged.",
            "",
            "10. Output contract",
            "Return JSON only.",

...
def _bundle_summary(bundle) -> dict[str, object]:
    return {
        "packet_version": bundle.packet_version,
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description=f"Build and inspect exact Agent Bundle V{PACKET_VERSION} directories")
    subparsers = parser.add_subparsers(dest="command", required=True)

    build_parser = subparsers.add_parser(
        "build", help=f"Import a finding and construct a coherent Packet V{PACKET_VERSION} bundle"
    )
    ...
    inspect_parser = subparsers.add_parser("inspect", help="Verify a published agent packet and print its handoff lists")
    inspect_parser.add_argument("--packet-dir", type=Path, required=True)

    open_parser = subparsers.add_parser("open-folder", help="Verify and open an agent packet directory in Explorer")
